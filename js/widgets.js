class ComprasWidget {
    ...
    render () {
        ...
        Array.from(botones).forEach((boton) => boton.addEventListener("click", this.agregarProducto));

        // Adicionando los handlers del evento click de las imágenes.
        const enlaces = this.parent.getElementsByClassName("ver-producto");
        Array.from(enlaces).forEach((boton) => boton.addEventListener("click", this.verProducto));
    }
